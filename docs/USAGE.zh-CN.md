...
| 控件 | API 模式 | WEB 模式 |
|---|---|---|
| 发送目标 | Zotero 中配置的模型预设 | ChatGPT、DeepSeek、ChatGLM、Kimi 或自定义 ChatGPT-like 网页 |
...
- **服务菜单**：切换 ChatGPT、DeepSeek、ChatGLM、Kimi 和已保存的自定义站点。最后的“管理第三方网页…”只打开 URL 管理，并恢复此前选中的服务。旧版保存的 `kimi.com` 自定义项会自动迁移为内置 Kimi，避免重复入口。
