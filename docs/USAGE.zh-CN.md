...
使用 **WEB 模式**时，点 **🔖 全文重点**会把论文上下文交给当前网页模型，并要求它在正常回答后附带结构化标注清单。侧边栏会保留网页模型的完整回答，同时用本地算法把清单中的原文定位到当前 PDF，显示可预览的“PDF 标注草稿”。只有点击**保存全部已定位条目**后才会写入 Zotero；这一流程不调用模型 API，也不改变 API 模式原有的工具调用、审批或 YOLO 行为。
...
2. 像普通对话一样提问并按 Enter。侧边栏依次显示准备、上传、提交、生成、同步进度；网页回答增长时会持续同步，不必等网页完成后才一次性出现。
3. `📄 原文`可把当前论文材料作为附件发送。隐藏模式优先使用网页真实文件输入框，不需要聚焦 Chrome；站点没有可用文件输入框时会明确报错，不会为了剪贴板粘贴而闪现浏览器。
4. 只有网页提供真实附件或可下载 URL 时，生成文件才会出现在 Zotero。正文里的 `sandbox:/...` 只是网页沙盒文字路径，不能当成下载文件。
