...
看到“访问验证”和滑块提示时，需要在专用 Chrome 中手动完成验证。
...
- 当前浏览器遇到过访问验证后，“后台隐藏”采用最小化，保留浏览器会话；
  不再为了隐藏窗口而重启为无界面 Chrome。没有遇到验证时沿用原后台模式。

这些处理避免插件打断人工验证，不保证网站不再要求验证。检测目前覆盖上述
整页滑块提示，不能据此认定其他验证码形式已得到相同处理。
