...
本文的当前处理对应 `v0.8.7`。从旧版升级时，请安装新版 XPI，并按账号窗口
提示安装或更新其配套 Web Agent ZIP。
...
**当前状态：用户已确认 chatglm.cn 不再受限。** 服务菜单恢复为
`ChatGLM（chatglm.cn）`，移除固定的“风控受限”标记。此前用户环境中曾出现
“验证失败，请刷新”，返回值为 `verifyResult: false`、`verifyCode: F001`；
以下保留实际遇到验证页面时的处理说明。

WEB 服务菜单另提供 **Z.ai（chat.z.ai）**，使用独立的页面适配器和会话记录。
游客可以文字聊天，上传附件需要在 Z.ai 网页登录；登录状态自动检测，无需关闭
Chrome。两个入口按各自网页状态运行。
...
**如何确认登录成功**：看 Zotero 账号窗口上方的状态框。游客时显示“游客模式
可用”；检测到网站登录后显示“Z.ai 已登录”。每次检查完成后约 1 秒再检查。
下方说明文字在打开窗口时按任务类型选择，之后不会随登录状态改变，不能作为
判断登录状态的依据。状态就绪后，点击“完成并隐藏”或“完成并保持显示”结束配置。

需要登录的是 `chat.z.ai` 网站账号，不要求开启 Chrome 账号同步；专用配置也
不会自动继承日常 Chrome 中的登录状态。若 Google 显示“此浏览器或应用可能
不安全”，说明该次登录被拒绝，不能把浏览器头像或说明文字当作登录成功。
...
解析完成后发给网页的是 MinerU 的 Markdown 全文（`full.md`），内容是纯文本：图片只
以 `images/xxx.jpg` 相对路径出现，图片文件本身不会随附件上传（它们只保存在本地
`zotero-ai-sidebar-mineru/<itemKey>/assets/`，供本地阅读页使用）。网页模型因此看不到
图和版式，问「图 3 说明了什么」这类问题会答不准；文字、LaTeX 公式和 HTML 表格不受
影响。需要按图提问时，请在问题里描述图表内容，或在网页中手动附加原始 PDF。
