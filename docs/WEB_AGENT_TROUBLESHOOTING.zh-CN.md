...
Google 登录可能拒绝自动化浏览器环境。Z.ai 账号配置现在先关闭受控的专用
Chrome，再用同一份资料目录打开普通 Chrome；这个登录阶段不启用远程调试，
不连接 Playwright，也不读取网页或提交任务。

用户在网页完成 Z.ai 登录后，关闭本次专用 Chrome 的全部窗口。插件随后重新
连接同一份资料，检查 Z.ai 的实际账号状态。游客仍可文字聊天，附件仍要求登录；
关闭窗口本身不会被当作登录成功。登录期间提前关闭 Zotero 账号对话框不会关闭
手动登录窗口，退出 Zotero 则会结束其专用浏览器进程。

普通 Chrome 登录仅用于 Z.ai；切换前必须结束现有 WEB 任务。本地隔离测试覆盖
登录期间不接管、关闭后检查、任务保护和退出清理。Google 真实登录及恢复自动化
后的会话有效性仍需实际账号验证，这项改动不保证网站免验证。
