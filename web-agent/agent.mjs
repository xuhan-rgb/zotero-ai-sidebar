...
// GLM and Z.ai keep visible Chrome sessions; background means minimized.
...
async function hideDedicatedBrowser(provider, customProvider) {
  ...
  accountWindowVisible = false;
  if (isGLMAdapter(adapter) || adapter.host === "chat.z.ai") {
    preserveBrowserSession = true;
    await minimizeDedicatedBrowser();
    return {
      ok: true, provider, browserOpen: true, configured, hidden: true,
...
async function ensureDedicatedBrowserMode(mode, adapter) {
  const keepVisible = isGLMAdapter(adapter) || adapter.host === "chat.z.ai";
  ...
  browserTransition = (async () => {
    ...
    const zaiBrowser =
      adapter.host !== "chat.z.ai" ||
      (await readFile(devToolsPortFile(), "utf8")
        .then((raw) => raw.split(/\r?\n/)[2] === "zai-visible")
        .catch(() => false));
    if (
      adapter.host === "chat.z.ai" &&
      runningMode &&
      (runningMode !== mode || !zaiBrowser) &&
      [...activeTasks.values()].some(
        (task) =>
          providerDefinition(task.provider, task.customProvider).host !==
          adapter.host,
      )
    ) {
      throw new Error("请先完成或取消其他 WEB 任务，再打开 Z.ai 账号配置");
    }
    preserveBrowserSession = keepVisible;
    if (runningMode === mode && zaiBrowser) {
      browserMode = runningMode;
      return;
    }
    if (runningMode) await stopDedicatedBrowser();
    await startDedicatedBrowser(mode, adapter);
  })();
  try {
    await browserTransition;
  } finally {
...
async function startDedicatedBrowser(mode, adapter) {
  await unlink(devToolsPortFile()).catch(() => undefined);
  if (adapter.host === "chat.z.ai") {
    await startZaiBrowser();
    browserMode = "visible";
    return;
  }
...
async function startZaiBrowser() {
  // Allocate a port dynamically, then pass the actual number to visible Chrome.
  // A competing process can claim it after release, so retry binding failures.
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    const child = spawn(
      config.chromePath,
      chromeLaunchArguments(config, "visible", port),
    ...
    try {
      const socket = await new Promise((resolve, reject) => {
        ...
        const timer = globalThis.setTimeout(
          () =>
            finish(
              new Error(
                "Z.ai 专用 Chrome 启动超时，请关闭已手动打开的专用 Chrome 后重试",
              ),
            ),
          20_000,
        );
        ...
        const onExit = () =>
          finish(
            new Error(
              "Z.ai 专用 Chrome 未启动调试接口，请关闭已手动打开的专用 Chrome 后重试",
            ),
          );
        const onData = (chunk) => {
          output = (output + chunk.toString()).slice(-16_384);
          if (
            /bind\(\) failed|Cannot start http server for devtools/i.test(
              output,
            )
          ) {
            const error = new Error("Z.ai 浏览器端口被占用");
      ...
      const endpoint = new globalThis.URL(socket);
      if (
        endpoint.hostname !== "127.0.0.1" ||
        Number(endpoint.port) !== port ||
        !endpoint.pathname.startsWith("/devtools/browser/")
      ) {
        throw new Error("Z.ai 浏览器返回了不匹配的调试接口");
      }
      // Use the endpoint announced by our child, never a service found solely
      // by probing the selected port. The marker preserves this mode on restart.
      await writeFile(
        devToolsPortFile(),
        `${port}\n${endpoint.pathname}\nzai-visible\n`,
        { mode: 0o600 },
      ...
      if (child.pid && child.exitCode == null && child.signalCode == null) {
        ...
        if (child.exitCode == null && child.signalCode == null) {
          throw new Error("Z.ai 专用 Chrome 未退出，已停止重试");
        }
      }
      await unlink(devToolsPortFile()).catch(() => undefined);
