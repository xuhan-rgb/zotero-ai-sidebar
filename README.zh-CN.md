...
> 预览版,以 GitHub **prerelease** 形式发布。稳定版用户可继续留在 v0.5.4;想试新的总览地图,可从 [v0.6.0-preview.1 release](https://github.com/xuhan-rgb/zotero-ai-sidebar/releases/tag/v0.6.0-preview.1) 下载 `.xpi`。

- **整篇论文「全文总览」地图**:对话与 Reader 之间的中间列——核心摘要叙述、按阶段(动机 / 方法 / 验证)分组的章节骨架(每节一句话主旨、`创新` / `效果锚点` 强调)、可折叠子章节,以及一张折叠的结构流程图。通过模型工具循环按需生成,不会自动整篇上传 PDF。
- **点击跳转 + 吸顶头部**:点击章节即跳到 PDF 对应位置——优先用 PDF 自带目录精确滚到章节顶部,没有目录时回退到文本匹配。头部带 ↶ 返回栈、↩ 回到`在读`、🔒 锁定(可浏览其它章节而不移动在读锚点)。
- **记住并同步阅读位置**:每篇论文的`在读`锚点跨重启保留,并随 `state.json` 同步;对话头部的 `🕘 最近阅读` 列表可跨整个库、跨机器跳回最近读过的论文。可用 `↗ 浏览器` 把总览导出为独立 HTML,或自动保存为子 HTML 附件。
- **内部重构**:`sidebar.ts` 从约 1.29 万行拆成 14 个聚焦模块(−38%),并删除约 790 行死代码——相对上述功能无行为变化。
...
- **记住并同步阅读位置**：每篇论文的`在读`锚点跨重启保留，并随 `state.json` 同步；对话头部的 `🕘 最近阅读` 列表可跨整个库、跨机器跳回最近读过的论文。
...
- **WebDAV 云同步**：通过单个 `state.json` 快照在任意 WebDAV 端点（如坚果云）推送 / 拉取——包含模型预设（含 API key）、显示设置、快捷提示词、联网/MCP 设置、翻译设置、AI 对话历史、逐句翻译缓存、完整 PDF 批注（高亮 / 下划线 / 笔记 / 墨迹）、逐条目全文总览，以及阅读位置（`在读`锚点 + 最近阅读列表）。
