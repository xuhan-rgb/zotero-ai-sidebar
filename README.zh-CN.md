...
- **`@` 选择论文素材**：在输入框里输入 `@`，可以选这篇论文的图片、表格和公式。PDF 已用 MinerU 解析时（包括 LaTeX 论文旁边打开的 PDF），你正在阅读的那一页的素材排在最前并标出「本页」，顶部标签可在 **本页 / 全部 / 图片 / 表格 / 公式** 之间切换。选图片会作为图片发送；选表格或公式只在输入框里留一个 `[表 #1]` 短标签，发送时才展开成完整的 LaTeX 源码。
- **`@` 支持 LaTeX 论文**：arXiv 图源里是 PDF 或 EPS 的插图会在选择时即时转成图片，并且会先展开 `\input` / `\include`，正文分散在多个 `.tex` 文件里的论文不再显示空列表。
- **WEB 图片真正上传**：输入框里附加的图片（包括用 `@` 选的）会随消息一起上传给网页，不再被静默丢弃。
- **WEB 使用须知**：输入行 `原文` 右边的浅灰标签列出 API 与 WEB 的差别（首先是 WEB 默认不发送论文插图），以及两个模式里发送图片、表格、公式的办法。论文卡片上重复的提示已移除。
- **升级方法**：安装 0.8.12 XPI 后重启 Zotero。已有设置、提示词编辑和 WEB 登录状态都会保留。
...
PDF 翻译可在插件设置的“沉浸阅读”区域调整：

- 默认翻译模型：沉浸阅读始终使用这里的账号、模型和思考程度；arXiv 全文翻译首次使用时从这里继承，之后可在全文翻译顶部独立修改并记忆
- 结果位置（句上、句下）/ 翻译框大小：用于沉浸翻译卡
- 结合上下句：翻译时带上前后各一句作上下文，更准
- 重点词对应：译文里高亮原文 ↔ 中文的关键词对，悬停互相点亮
- 切句快捷键（默认 `Enter` / `Shift+Enter`）、选区快捷翻译键（默认空格）、聚焦追问键（默认 `/`）、收起工具栏键（默认 `h`）均可改

Quick Ask 默认用 `Alt+Q` 打开。首次使用时采用当前 AI 对话模型，之后会独立记住窗口内选择的账号、模型和思考程度；这些选择不会反向修改研究对话或默认翻译模型。
...
> 以下 Z.ai、自动端口分配和登录检测流程适用于 `v0.8.9`。从旧版升级时，请安装新版 XPI，并按账号窗口提示安装或更新其配套 Web Agent ZIP。
...
- **arXiv 全文翻译**：有 `LaTeX 源` 徽章时可打开全文翻译阅读页，按中英 / 中文 / 英文及左右 / 逐段方式阅读；顶部模型名称可展开账号、模型和思考强度设置。首次从默认翻译模型继承，修改后由全文翻译独立记忆。
...
- **快捷提示词落在数据目录**：内置模板、自定义按钮和「普通选区提问后生成建议注释」开关保存在 `zotero-ai-sidebar-quick-prompts.json`，与聊天记录一样放在 Zotero 数据目录，不写入 `prefs.js`。
