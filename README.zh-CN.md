...
- **`@` 选择论文素材**：在输入框里输入 `@`，可以选这篇论文的图片、表格和公式。PDF 已用 MinerU 解析时（包括 LaTeX 论文旁边打开的 PDF），你正在阅读的那一页的素材排在最前并标出「本页」，顶部标签可在 **本页 / 全部 / 图片 / 表格 / 公式** 之间切换。选图片会作为图片发送，选表格或公式则插入对应的 LaTeX 源码。
